...
def main():
    ...
    keypoints = [
        ia.KeypointsOnImage(
            [ia.Keypoint(x=50, y=40), ia.Keypoint(x=70, y=38), ia.Keypoint(x=62, y=52)],
            shape=images[0].shape,
        ),
        ia.KeypointsOnImage(
            [ia.Keypoint(x=55, y=32), ia.Keypoint(x=42, y=95), ia.Keypoint(x=75, y=89)],
    ...
    augmenters = [
        ...
        iaa.Sometimes(
            0.5, iaa.AdditiveGaussianNoise(scale=0.1 * 255), name="Sometimes"
        ),
        ...
        iaa.AddToHueAndSaturation(
            (-20, 20), per_channel=True, name="AddToHueAndSaturation"
        ),
        ...
        iaa.CoarseSaltAndPepper(
            p=0.05, size_percent=(0.01, 0.1), name="CoarseSaltAndPepper"
        ),
        ...
        iaa.PiecewiseAffine(
            scale=0.03, nb_rows=(2, 6), nb_cols=(2, 6), name="PiecewiseAffine"
        ),
        ...
        iaa.SimplexNoiseAlpha(
            first=iaa.EdgeDetect(1.0), per_channel=False, name="SimplexNoiseAlpha"
        ),
        iaa.FrequencyNoiseAlpha(
            first=iaa.EdgeDetect(1.0), per_channel=False, name="FrequencyNoiseAlpha"
        ),
    ]

    augmenters.append(
        iaa.Sequential(
            [iaa.Sometimes(0.2, aug.copy()) for aug in augmenters], name="Sequential"
        )
    )
    augmenters.append(
        iaa.Sometimes(0.5, [aug.copy() for aug in augmenters], name="Sometimes")
    )
