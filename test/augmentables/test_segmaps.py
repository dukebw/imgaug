...
class TestSegmentationMapsOnImage___init__(unittest.TestCase):
        ...
        for dtype, ndim, img_shape in gen:
            with self.subTest(dtype=dtype, ndim=ndim, shape=img_shape):
                ...
                assert np.array_equal(
                    segmap.arr, arr.reshape((3, 3, 1)).astype(np.int32)
                )

                if ndim == 3:
                    arr = np.array(
                        [[0, 0, 1], [0, 2, 1], [1, 3, 1]], dtype=dtype
                    ).reshape((3, 3, 1))
    ...
    def test_legacy_support_for_float32_2d(self):
        ...
        assert len(caught_warnings) == 1
        assert "Got a float array as the segmentation map in" in str(
...
class TestSegmentationMapsOnImage_draw(unittest.TestCase):
    ...
    def test_with_two_classes(self):
        ...
        col1 = self.col(1)
        expected = np.uint8(
            [[col0, col1, col1], [col0, col1, col1], [col0, col1, col1]]
        )
...
class TestSegmentationMapsOnImage_draw_on_image(unittest.TestCase):
    ...
    def test_alpha_only_segmap_is_visible(self):
        # only segmap visible
        observed = self.segmap.draw_on_image(
            self.image, alpha=1.0, draw_background=True
        )
        col0 = self.col(0)
        col1 = self.col(1)
        expected = np.uint8(
    ...
    def test_non_binary_alpha_with_draw_background_true(self):
        ...
        d_max = np.max(
            np.abs(observed[0].astype(np.float32) - expected.astype(np.float32))
        )
        assert isinstance(observed, list)
        assert len(observed) == 1
        assert observed[0].shape == expected.shape
    ...
    def test_resize_image_to_segmentation_map(self):
        ...
        observed = segmap.draw_on_image(
            image, alpha=a1, draw_background=True, resize="image"
        )
