...
def main():
    ...
    time_end = time.time()
    print(
        "<%s> Finished without errors in %.4fs."
        % (
            __file__,
            time_end - time_start,
        )
...
def test_draw_text():
    ...
    img_text = ia.draw_text(
        img, y=5, x=5, text="---------", size=10, color=[255, 255, 255]
    )
    ...
    img_text1 = ia.draw_text(
        img, y=5, x=5, text="XXXXXXX", size=10, color=[255, 255, 255]
    )
    img_text2 = ia.draw_text(
        img, y=5, x=50, text="XXXXXXX", size=10, color=[255, 255, 255]
    )
    first_col1 = None
    first_col2 = None
    for i in range(img.shape[1]):
    ...
    img_text2 = ia.draw_text(
        img, y=50, x=5, text="XXXXXXX", size=10, color=[255, 255, 255]
    )
    first_row1 = None
    first_row2 = None
    for i in range(img.shape[0]):
    ...
    img_text_small = ia.draw_text(
        img, y=5, x=5, text="X", size=10, color=[255, 255, 255]
    )
    img_text_large = ia.draw_text(
        img, y=5, x=5, text="X", size=50, color=[255, 255, 255]
    )
...
def test_imresize_many_images():
    ...
    for c in [1, 3]:
        ...
        for images_this_iter in [
            images,
            list(images),
        ]:  # test for ndarray and list(ndarray) input
            for interpolation in interpolations:
                images_same_observed = ia.imresize_many_images(
                    images_this_iter, (16, 16), interpolation=interpolation
                )
                for image_expected, image_observed in zip(
                    images_this_iter, images_same_observed
                ):
                    diff = np.abs(
                        image_expected.astype(np.int32)
                        - image_observed.astype(np.int32)
                    )
            ...
            for interpolation in interpolations:
                ...
                for image_expected, image_observed in zip(
                    images_small, images_small_observed
                ):
                    diff = np.abs(
                        image_expected.astype(np.int32)
                        - image_observed.astype(np.int32)
            ...
            for interpolation in interpolations:
                ...
                for image_expected, image_observed in zip(
                    images_large, images_large_observed
                ):
                    diff = np.abs(
                        image_expected.astype(np.int32)
                        - image_observed.astype(np.int32)
    ...
    for dtype in [np.uint8, np.uint16, np.int8, np.int16, np.int32]:
        ...
        for value in [min_value, max_value]:
            ...
            expected[1, :] = value
            expected[2, :] = value
            image_rs = ia.imresize_many_images(
                [image], (3, 3), interpolation="nearest"
            )[0]
    ...
    for c in [-1, 1, 3]:
        ...
        for interpolation in interpolations:
            for image, image_expected in zip(images, images_small):
                ...
                diff = np.abs(
                    image_expected.astype(np.int32) - image_observed.astype(np.int32)
                )
                diff_fraction = np.sum(diff) / (image_observed.size * 255)
                assert diff_fraction < 0.5
    ...
    def test_2d_image(self):
        ...
        for subtable_descr, subtable in tables:
            with self.subTest(table_type=subtable_descr):
                image = np.uint8(
                    [[0, 50, 100, 245, 254, 255], [1, 51, 101, 246, 255, 0]]
                )
