...
class Test_get_imgcorrupt_subset(unittest.TestCase):
    @unittest.skipUnless(
        SUPPORTS_LIBRARY, "imagecorruptions can only be tested for python 3.5+"
    )
        ...
        for subset in subset_names:
            with self.subTest(subset=subset):
                ...
                for func_name, func in zip(func_names, funcs):
                    assert (
                        getattr(iaa.imgcorruptlike, "apply_%s" % (func_name,)) is func
                    )
...
class _CompareFuncWithImageCorruptions(unittest.TestCase):
    ...
    @classmethod
    def _run_single_comparison_test(
        cls, fname, image_imgaug, image_imgcor, severity, seed
    ):
    ...
    def _generate_augmented_images(
        ...
        func_imgaug = getattr(iaa.imgcorruptlike, "apply_%s" % (fname,))
...
class Test_apply_functions(_CompareFuncWithImageCorruptions):
    ...
    def test_apply_frost(self):
        self._test_by_comparison_with_imagecorruptions(
            "frost", severities=[1, 5], seeds=[1, 5, 10]
        )
...
class TestAugmenters(unittest.TestCase):
    ...
    def _test_augmenter(cls, augmenter_name, func_expected, dependent_on_seed):
        ...
        image = (
            np.mod(np.arange(32 * 32 * 3), 256).reshape((32, 32, 3)).astype(np.uint8)
        )
        ...
        with iap.no_prefetching():
            ...
            image_aug_exp = func_expected(
                image, severity=severity, seed=rng.generate_seed_()
            )
    ...
    def test_gaussian_noise(self):
        self._test_augmenter(
            "GaussianNoise", iaa.imgcorruptlike.apply_gaussian_noise, True
        )
    ...
    def test_impulse_noise(self):
        self._test_augmenter(
            "ImpulseNoise", iaa.imgcorruptlike.apply_impulse_noise, True
        )

    def test_speckle_noise(self):
        self._test_augmenter(
            "SpeckleNoise", iaa.imgcorruptlike.apply_speckle_noise, True
        )

    def test_gaussian_blur(self):
        self._test_augmenter(
            "GaussianBlur", iaa.imgcorruptlike.apply_gaussian_blur, False
        )
    ...
    def test_defocus_blur(self):
        self._test_augmenter(
            "DefocusBlur", iaa.imgcorruptlike.apply_defocus_blur, False
        )
    ...
    def test_jpeg_compression(self):
        self._test_augmenter(
            "JpegCompression", iaa.imgcorruptlike.apply_jpeg_compression, False
        )
