...
def test_determinism():
    ...
    for aug in augs:
        ...
        assert array_equal_lists(
            images_aug1, images_aug2
        ), "Images (1, 2) expected to be identical for %s" % (aug.name,)

        assert array_equal_lists(
            images_aug3, images_aug4
        ), "Images (3, 4) expected to be identical for %s" % (aug.name,)

        assert not array_equal_lists(
            images_aug1, images_aug3
    ...
    def test_many_augmenters(self):
        ...
        augs = [
            ...
            iaa.DirectedEdgeDetect(
                alpha=(0.0, 0.1), direction=0, name="DirectedEdgeDetect"
            ),
            iaa.Fliplr(0.5, name="Fliplr"),
            iaa.Flipud(0.5, name="Flipud"),
            iaa.Affine(translate_px=(-5, 5), name="Affine-translate-px"),
            iaa.Affine(
                translate_percent=(-0.05, 0.05), name="Affine-translate-percent"
            ),
            ...
            iaa.BlendAlphaElementwise(
                (0.0, 0.1), iaa.Add(10), name="BlendAlphaElementwise"
            ),
        ...
        for aug in augs:
            ...
            for i in sm.xrange(10):
                ...
                if len(ds) == 0:
                    print(
                        "[INFO] No valid keypoints found for '%s' "
...
def test_dtype_preservation():
    ...
    augs = [
        ...
        (
            iaa.Sequential([iaa.Identity(), iaa.Identity()], name="SequentialNoop"),
            default_dtypes,
        ),
        (
        ...
        (
            iaa.Sequential(
                [iaa.Add((-5, 5)), iaa.AddElementwise((-5, 5))], name="Sequential"
            ),
            _not_dts([np.uint32, np.int32, np.float64]),
        ),
        (
            iaa.SomeOf(
                1, [iaa.Add((-5, 5)), iaa.AddElementwise((-5, 5))], name="SomeOf"
            ),
            _not_dts([np.uint32, np.int32, np.float64]),
        ),
        (
        ...
        (
            iaa.BlendAlphaSimplexNoise(
                iaa.Identity(), name="BlendAlphaSimplexNoiseIdentity"
            ),
            _not_dts([np.float64]),
        ),  # float64 requires float128 support
        (
