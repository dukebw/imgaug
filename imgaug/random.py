...
def _derive_generators_np117_(generator, n):
    ...
    # needed.)
    seed_ = generator.integers(
        SEED_MIN_VALUE, SEED_MAX_VALUE, dtype="int32", size=(2,)
    )[-1]
...
def _is_generator_equal_to_np117(generator, other_generator):
    assert (
        generator.__class__ is other_generator.__class__
    ), "Expected both rngs to have the same class, " "got types '%s' and '%s'." % (
        type(generator),
        type(other_generator),
    )
...
def polyfill_integers(
    generator, low, high=None, size=None, dtype="int32", endpoint=False
):
