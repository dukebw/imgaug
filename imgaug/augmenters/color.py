...
def _get_opencv_attr(attr_names):
    ...
    ia.warn(
        "Could not find any of the following attributes in cv2: %s. "
        "This can cause issues with colorspace transformations." % (attr_names)
    )
    return None
...
_CSPACE_OPENCV_CONV_VARS = {
    ...
    # GRAY
    # YCrCb
    (CSPACE_YCrCb, CSPACE_RGB): _get_opencv_attr(
        ["COLOR_YCrCb2RGB", "COLOR_YCR_CB2RGB"]
    ),
    (CSPACE_YCrCb, CSPACE_BGR): _get_opencv_attr(
        ["COLOR_YCrCb2BGR", "COLOR_YCR_CB2BGR"]
    ),
    # HSV
...
def change_colorspace_(image, to_colorspace, from_colorspace=CSPACE_RGB):
    ...
    **Supported dtypes**:
    ...
    assert (
        image.shape[2] == 3
    ), "Expected number of channels to be three, " "got %d channels (shape %s)." % (
        image.shape[2],
        image.shape,
    )
    ...
    **Supported dtypes**:
    ...
    def _hs_to_images_(self, images_hs, images_hsv):
        ...
        for i, hs_aug in enumerate(images_hs):
            ...
            hue_and_sat_proj.append(
                np.stack([hue_aug, sat_aug, images_hsv[i][:, :, 2]], axis=-1)
            )
    ...
    def __str__(self):
        return (
            ...
            % (self.from_colorspace, self.name, self.children, self.deterministic)
        )
    ...
    def __init__(
        ...
        if mul is not None:
            assert mul_hue is None, (
                ...
                "It is set to: %s (type: %s)." % (str(mul_hue), type(mul_hue))
            )
    ...
    def _draw_samples(self, augmentables, random_state):
        ...
        if self.value is not None:
            ...
            samples = self.value.draw_samples(
                (nb_images, 2), random_state=rss[1]
            ).astype(np.int32)
        ...
        samples_hue = ((samples_hue.astype(np.float32) / 255.0) * (360 / 2)).astype(
            np.int32
        )
    ...
    def _handle_value_arg(cls, value, value_hue, value_saturation):
        if value is not None:
            assert value_hue is None, (
                ...
                "It is set to: %s (type: %s)." % (str(value_hue), type(value_hue))
            )
...
class ChangeColorspace(meta.Augmenter):
    ...
    **Supported dtypes**:
    ...
    CV_VARS = {
        ...
        # Lab
        "Lab2RGB": (
            cv2.COLOR_Lab2RGB if hasattr(cv2, "COLOR_Lab2RGB") else cv2.COLOR_LAB2RGB
        ),
        "Lab2BGR": (
            cv2.COLOR_Lab2BGR if hasattr(cv2, "COLOR_Lab2BGR") else cv2.COLOR_LAB2BGR
        ),
    }
    ...
    def __init__(
        ...
        if ia.is_string(to_colorspace):
            ...
            assert all_strings, (
                ...
                "Got types %s." % (", ".join([str(type(v)) for v in to_colorspace]))
            )
            all_valid = all(
                [(colorspace in CSPACE_ALL) for colorspace in to_colorspace]
            )
            assert all_valid, (
                "Expected list of 'to_colorspace' to only contain strings "
                "that are in %s. Got strings %s." % (CSPACE_ALL, to_colorspace)
            )
            ...
            raise Exception(
                ...
                "strings or StochasticParameter, got %s." % (type(to_colorspace),)
            )
        ...
        assert ia.is_string(
            from_colorspace
        ), "Expected from_colorspace to be a single string, " "got type %s." % (
            type(from_colorspace),
        )
        assert (
            from_colorspace in CSPACE_ALL
        ), "Expected from_colorspace to be one of: %s. Got: %s." % (
            ", ".join(CSPACE_ALL),
            from_colorspace,
        )
        assert (
            from_colorspace != CSPACE_GRAY
    ...
    def _augment_batch_(self, batch, random_state, parents, hooks):
        ...
        for i, image in enumerate(images):
            ...
            if alpha <= self.eps or self.from_colorspace == to_colorspace:
                ...
                image_aug = change_colorspace_(
                    image, to_colorspace, self.from_colorspace
                )
...
class _AbstractColorQuantization(meta.Augmenter):
    ...
    def _augment_single_image(self, image, counts, random_state):
        ...
        assert image.shape[-1] in [
            ...
            3,
            4,
        ], "Expected image with 1, 3 or 4 channels, " "got %d (shape: %s)." % (
            image.shape[-1],
            image.shape,
        )
...
def quantize_colors_kmeans(image, n_colors, n_max_iter=10, eps=1.0):
    ...
    """
    return quantize_kmeans(
        arr=image, nb_clusters=n_colors, nb_max_iter=n_max_iter, eps=eps
    )
...
def quantize_kmeans(arr, nb_clusters, nb_max_iter=10, eps=1.0):
    ...
    **Supported dtypes**:
    ...
    assert arr.ndim in [
        2,
        3,
    ], "Expected two- or three-dimensional array shape, " "got shape %s." % (arr.shape,)
...
def quantize_uniform(arr, nb_bins, to_bin_centers=True):
    ...
    **Supported dtypes**:
    ...
    """
    return quantize_uniform_(
        np.copy(arr), nb_bins=nb_bins, to_bin_centers=to_bin_centers
    )
...
class _QuantizeUniformLUTTable(object):
    ...
    def _generate_quantize_uniform_table(cls, centerize):
        ...
        table_all_nb_bins = np.clip(np.round(table_all_nb_bins), 0, 255).astype(
            np.uint8
        )
