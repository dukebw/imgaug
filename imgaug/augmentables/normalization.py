...
def _preprocess_shapes(shapes):
    ...
    assert isinstance(
        shapes, list
    ), "Expected 'shapes' to be None or ndarray or list, got type %s " "instead." % (
        type(shapes),
    )
    result = []
...
def _assert_single_array_last_dim_exactly(arr, size, to_ntype):
    if arr.shape[-1] != size:
        raise ValueError(
            ...
            "shape: %s)." % (to_ntype, size, arr.shape[-1], str(arr.shape))
        )
...
def normalize_heatmaps(inputs, shapes=None):
    ...
    if ntype == "array[float]":
        ...
        return [
            HeatmapsOnImage(attr_i, shape=shape_i)
    ...
    assert (
        ntype == "iterable-HeatmapsOnImage"
...
def normalize_segmentation_maps(inputs, shapes=None):
    ...
    if ntype in ["array[int]", "array[uint]", "array[bool]"]:
        _assert_single_array_ndim(
            inputs, 4, "(N,H,W,#SegmapsPerImage)", "SegmentationMapsOnImage"
        )
        _assert_exactly_n_shapes_partial(n=len(inputs))
        if ntype == "array[bool]":
            return [
    ...
    if ntype in ["iterable-array[int]", "iterable-array[uint]", "iterable-array[bool]"]:
        _assert_many_arrays_ndim(
            inputs, 3, "(H,W,#SegmapsPerImage)", "SegmentationMapsOnImage"
        )
        _assert_exactly_n_shapes_partial(n=len(inputs))
    ...
    assert (
        ntype == "iterable-SegmentationMapsOnImage"
    ), "Got unknown normalization type '%s'." % (ntype,)
    return inputs  # len allowed to differ from len of images
...
def normalize_keypoints(inputs, shapes=None):
    ...
    if ntype == "iterable-tuple[number,size=2]":
        _assert_exactly_n_shapes_partial(n=1)
        return [
            KeypointsOnImage([Keypoint(x=x, y=y) for x, y in inputs], shape=shapes[0])
        ]
    ...
    assert (
        ntype == "iterable-iterable-Keypoint"
    ), "Got unknown normalization type '%s'." % (ntype,)
    _assert_exactly_n_shapes_partial(n=len(inputs))
    return [
        KeypointsOnImage(attr_i, shape=shape) for attr_i, shape in zip(inputs, shapes)
    ]
...
def normalize_bounding_boxes(inputs, shapes=None):
    ...
    assert (
        ntype == "iterable-iterable-BoundingBox"
    ), "Got unknown normalization type '%s'." % (ntype,)
    _assert_exactly_n_shapes_partial(n=len(inputs))
    return [
    ...
    if ia.is_np_array(images_old):
        ...
        if images_old.ndim == 3:
            assert images.shape[3] == 1, (
                ...
                "Got instead C=%d and shape %s" % (images.shape[3], images.shape)
            )
            return images[..., 0]
        return images
        ...
        for image, image_old in zip(images, images_old):
            if image_old.ndim == 2:
                assert image.shape[2] == 1, (
                    ...
                    "Got instead C=%d and shape %s." % (image.shape[2], image.shape)
                )
                result.append(image[:, :, 0])
            else:
                assert image_old.ndim == 3, (
                    "Expected 'image_old' to be three-dimensional, got %d "
                    "dimensions and shape %s." % (image_old.ndim, image_old.shape)
                )
                result.append(image)
        return result
    raise ValueError(
...
def invert_normalize_heatmaps(heatmaps, heatmaps_old):
    ...
    if ntype == "None":
        assert heatmaps is None, (
            ...
            "'heatmaps_old' being None. Got type %s instead." % (type(heatmaps),)
        )
        return heatmaps
    if ntype == "array[float]":
        ...
        return restore_dtype_and_merge(
            [hm_i.arr_0to1 for hm_i in heatmaps], input_dtype
        )
    if ntype == "HeatmapsOnImage":
        assert len(heatmaps) == 1, (
            "Expected as many heatmaps after normalization as before "
            "normalization. Got %d (after) and %d (before)." % (len(heatmaps), 1)
        )
        return heatmaps[0]
    if ntype == "iterable[empty]":
        assert heatmaps is None, (
    ...
    if ntype == "iterable-array[float]":
        ...
        return [
            restore_dtype_and_merge(hm_i.arr_0to1, input_dtype) for hm_i in heatmaps
        ]
...
def invert_normalize_keypoints(keypoints, keypoints_old):
    ...
    if ntype == "None":
        assert keypoints is None, (
            ...
            "'keypoints_old' being None. Got type %s instead." % (type(keypoints),)
        )
        return keypoints
    if ntype in ["array[float]", "array[int]", "array[uint]"]:
        ...
        return restore_dtype_and_merge(
            [kpsoi.to_xy_array() for kpsoi in keypoints], input_dtype
        )
    if ntype == "tuple[number,size=2]":
        assert len(keypoints) == 1, (
            "Expected a single KeypointsOnImage instance after normalization "
...
def invert_normalize_bounding_boxes(bounding_boxes, bounding_boxes_old):
    ...
    if ntype in ["array[float]", "array[int]", "array[uint]"]:
        assert len(bounding_boxes) == 1, (
            ...
            "normalization. Got %d instances instead." % (len(bounding_boxes),)
        )
    ...
    if ntype == "BoundingBoxesOnImage":
        assert len(bounding_boxes) == 1, (
            ...
            "before normalization. Got %d instances instead." % (len(bounding_boxes),)
        )
    ...
    if ntype == "iterable-iterable[empty]":
        assert bounding_boxes is None, (
            "Expected bounding boxes after normalization to be None, due "
            "to the bounding boxes before normalization being an empty "
            "iterable of iterables. Got type %s instead." % (type(bounding_boxes),)
        )
...
def _invert_normalize_polygons_and_line_strings(
    ...
    if ntype == "iterable-iterable[empty]":
        assert inputs is None, (
            "Expected polygons/line strings after normalization to be None, "
            "due to the polygons/line strings before normalization being an "
            "empty iterable of iterables. Got type %s instead." % (type(inputs),)
        )
        return inputs_old[:]
    if ntype in [
        "iterable-iterable-array[float]",
    ...
    assert (
        ntype == "iterable-iterable-iterable-Keypoint"
    ), "Got unknown normalization type '%s'." % (ntype,)
    return [
        [
            [Keypoint(x=point[0], y=point[1]) for point in get_points_func(entity)]
...
def _nonempty_info_to_type_str(nonempty, success, parents):
    assert len(parents) <= 4, "Expected 'parents' to be <=4, got %d." % (len(parents),)
    parent_iters = ""
    if len(parents) > 0:
    ...
    if ia.is_np_array(nonempty):
        ...
        return "%sarray[%s]" % (
            parent_iters,
            kind_map[kind] if kind in kind_map else kind,
        )
    ...
    assert isinstance(
        nonempty, object
    ), "Expected 'nonempty' to be an object, got type %s." % (type(nonempty),)
